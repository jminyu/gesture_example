...
#include <assert.h>
#include <strsafe.h>
#pragma comment(lib,"D:\grdll.lib")


extern "C" __declspec(dllimport) float DBN(int gesturecode,int len,int motion_chain[]);
extern "C" __declspec(dllimport) int MDirection_feature(float current_coord[], float past_coord[]);
extern "C" __declspec(dllimport) int Main_Axis(float distance[]);
extern "C" __declspec(dllimport) int Recognition_DBN(int length, int motion_chain[]);
...
int motion_chain_right[90] = {0,};
int motion_chain_left[90] = {0,};

float prighthand[3]; //coordinate of previous right hand
float plefthand[3];
float phead[3];
...
/// <param name="windowHeight">height (in pixels) of output buffer</param>s
void CSkeletalViewerApp::Nui_DrawSkeleton( const NUI_SKELETON_DATA & skel, int windowWidth, int windowHeight ) //experimental function (durlqnxj)
{      
    int i;
	///array for object coordinate
	///a[0]:x, a[1]:y, a[2]:z
	float crighthand[3]; //coordinate of current right hand
	float clefthand[3]; //coordinate of current left hand
	float chead[3]; //coordinate of current head
	 //coordinate of previous head
	...
	*/
	...
	m_Points[3] = SkeletonToScreen(skel.SkeletonPositions[3], windowWidth, windowHeight); // Head coordinate  - X and y coordinate is provided
	m_Points[7] = SkeletonToScreen(skel.SkeletonPositions[7], windowWidth, windowHeight); // left hand coordinate  - X and y coordinate is provided
	m_Points[11] = SkeletonToScreen(skel.SkeletonPositions[11], windowWidth, windowHeight); //right hand coordinate  - X and y coordinate is provided
	chead[0] = skel.SkeletonPositions[3].x;
	chead[1] = skel.SkeletonPositions[3].y;
	chead[2] = skel.SkeletonPositions[3].z;

	crighthand[0] = skel.SkeletonPositions[7].x;
	crighthand[1] = skel.SkeletonPositions[7].y;
	crighthand[2] = skel.SkeletonPositions[7].z;

	clefthand[0] = skel.SkeletonPositions[11].x;
	clefthand[1] = skel.SkeletonPositions[11].y;
	clefthand[2] = skel.SkeletonPositions[11].z;
	
	if(state==0){
		for(int k=0;k<3;k++){
			phead[k] = chead[k];
			prighthand[k] = crighthand[k];
			plefthand[k] = clefthand[k];

		}
		//1:segmentation for gesture
		mdf_right = MDirection_feature(crighthand,prighthand);
		mdf_left = MDirection_feature(clefthand,plefthand);
		
		PostMessageW( m_hWnd, WM_USER_UPDATE_RIGHTCOORD, IDC_RIGHT,mdf_right );
		PostMessageW( m_hWnd, WM_USER_UPDATE_LEFTCOORD, IDC_LEFT,mdf_left );
		...
		if(mdf_right!=0){
			//motion is validate
			if(right_count<90){
			motion_chain_right[right_count++]=mdf_right;
			}else{
				for(int t=0;t<89;t++){
					motion_chain_right[t]=motion_chain_right[t+1];
				}
				motion_chain_right[89]=mdf_right;
			}

		}else{//'No motion'
			if(right_count!=0){//There is no motion
				//PostMessageW( m_hWnd, WM_USER_UPDATE_RIGHTGESTURE,IDC_RGESTURE,right_count);
				PostMessageW( m_hWnd, WM_USER_UPDATE_RIGHTGESTURE,IDC_RGESTURE,Recognition_DBN(right_count,motion_chain_right));
				memset(motion_chain_right,0,90);
				right_count = 0;
			}else{
				//PostMessageW( m_hWnd, WM_USER_UPDATE_RIGHTGESTURE,IDC_RGESTURE,right_count);
				//PostMessageW( m_hWnd, WM_USER_UPDATE_RIGHTGESTURE,IDC_RGESTURE,0);
				right_count = 0;	
			}
		}
		...
		if(mdf_left!=0){
			//motion is validate
			if(left_count<90){
			motion_chain_left[left_count++]=mdf_left;
			}else{
				for(int t=0;t<89;t++){
					motion_chain_left[t]=motion_chain_left[t+1];
				}
				motion_chain_left[89]=mdf_left;
			}
		}else{
			if(left_count!=0){
				//PostMessageW( m_hWnd, WM_USER_UPDATE_LEFTGESTURE,IDC_LGESTURE,left_count);
				PostMessageW( m_hWnd, WM_USER_UPDATE_LEFTGESTURE,IDC_LGESTURE,Recognition_DBN(left_count,motion_chain_left));
				memset(motion_chain_left,0,90);
				left_count = 0;			
			}else{
				//PostMessageW( m_hWnd, WM_USER_UPDATE_LEFTGESTURE,IDC_LGESTURE,0);
				left_count = 0;	
			}
			//'No motion'
		}
		//2:feature extraction for constructing motion chain
		//3:recognition using DBN
		state=1;
	}else{
		mdf_right = MDirection_feature(crighthand,prighthand);
		mdf_left = MDirection_feature(clefthand,plefthand);
