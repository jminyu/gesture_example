...
LRESULT CALLBACK CSkeletalViewerApp::WndProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
    switch(message)
    {
        case WM_INITDIALOG:
        {
            ...
            LOGFONT lf;
			LOGFONT cf;//font for printing coordinate
            GetObject( (HFONT)GetStockObject(DEFAULT_GUI_FONT), sizeof(lf), &lf );
			GetObject( (HFONT)GetStockObject(DEFAULT_GUI_FONT), sizeof(cf), &cf );
            lf.lfHeight *= 4;//original value is 4
			cf.lfHeight *= 2;
            m_hFontFPS = CreateFontIndirect(&lf);
			m_hFontCOORDS = CreateFontIndirect(&cf);

            SendDlgItemMessageW(hWnd, IDC_FPS, WM_SETFONT, (WPARAM)m_hFontFPS, 0);

			SendDlgItemMessageW(hWnd, IDC_RIGHT, WM_SETFONT, (WPARAM)m_hFontCOORDS, 0);
			SendDlgItemMessageW(hWnd, IDC_LEFT, WM_SETFONT, (WPARAM)m_hFontCOORDS, 0);
			SendDlgItemMessageW(hWnd, IDC_HEAD, WM_SETFONT, (WPARAM)m_hFontCOORDS, 0);

			SendDlgItemMessageW(hWnd, IDC_LGESTURE, WM_SETFONT, (WPARAM)m_hFontCOORDS, 0);
			SendDlgItemMessageW(hWnd, IDC_RGESTURE, WM_SETFONT, (WPARAM)m_hFontCOORDS, 0);
        ...
        break;
		//hand update
		case WM_USER_UPDATE_RIGHTCOORD:
        {
            ::SetDlgItemInt( m_hWnd, static_cast<int>(wParam), static_cast<int>(lParam), FALSE );
        }
        break;

		case WM_USER_UPDATE_LEFTCOORD:
        {
            ::SetDlgItemInt( m_hWnd, static_cast<int>(wParam), static_cast<char>(lParam), FALSE );
        }
        break;

		case WM_USER_UPDATE_HEADCOORD:
        {
            ::SetDlgItemInt( m_hWnd, static_cast<int>(wParam), static_cast<int>(lParam), FALSE );
        }
        break;

		case WM_USER_UPDATE_RIGHTGESTURE:
        {
            ::SetDlgItemInt( m_hWnd, static_cast<int>(wParam), static_cast<int>(lParam), FALSE );
        }
        break;

		case WM_USER_UPDATE_LEFTGESTURE:
        {
            ::SetDlgItemInt( m_hWnd, static_cast<int>(wParam), static_cast<int>(lParam), FALSE );
        }
        break;
